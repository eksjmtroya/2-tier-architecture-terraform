# ? Los EIP son Ips elasticas (IPs públicas, proporcionadas por AES) y se pueden asociar a instancias EC2 o a servicios como NAT Gateways
# ? Como podmeos ve en la creación de los recursos nat-a y nat-b se requiere un allocation-id que son las eip creada para cada NAT
...
resource "aws_nat_gateway" "nat-a" {
  allocation_id = aws_eip.eip-nat-a.id #* Uso de la EIP-A
...
resource "aws_nat_gateway" "nat-b" {
  allocation_id = aws_eip.eip-nat-b.id  #* Uso de la EIP-B
...
# ? JUsto arriba hemos creado una tabla de ruta que saca "cualquier IP/destino", por la NAT que hemos creado para la zona A, y abajo se asociad la rt creada a la private
# ? subenet 3a
...
#? Asociamos las otras subnets privadas también para que salgan  por la NAT
